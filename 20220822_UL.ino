...
#define calibration_factor -21650.00 //wsp. kalibracji
#define DOUT  32
#define CLK  33
#define transistor 13
#define resetScale 14
...
void setup() {
  ...
  //client.init("matek", "", "http://51.68.141.235:8088/");
  ...
  pinMode(resetScale,INPUT_PULLUP);
  scale.tare();
...
void loop() {
  ...
  {
    char tareSign = Serial.read();
    if(tareSign == 't') {
      Serial.println("TARUJE");
      scale.tare();
    }
  }
  
  if(!digitalRead(resetScale)){
    scale.tare(); 
  }
  ...
  Hive h1 = Hive(1234, ID, tempC, humidity, weight);
